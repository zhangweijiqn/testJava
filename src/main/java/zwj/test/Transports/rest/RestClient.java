...
import zwj.test.Transports.Account;
import  zwj.test.Transports.transport.*;

import zwj.test.Transports.util.DateUtils;
import zwj.test.Transports.util.IOUtils;
import zwj.test.Transports.util.SvnRevisionUtils;
...
import zwj.test.Transports.transport.Request;
...
public class RestClient {
   ...
   * @throws Exception
   */
  public <T> T request(Class<T> clazz, String resource, String method) throws Exception {
...
//      throw new Exception("Can't bind xml to " + clazz.getName(), e);
//    }
  ...
  public Response request(String resource, String method, Map<String, String> params,
                          Map<String, String> headers,
                          InputStream body, long bodyLen) throws Exception {
    int retryTimes = 0;
    if (method.equalsIgnoreCase(Request.Method.GET.toString()) || method
        .equalsIgnoreCase(Request.Method.HEAD.toString())) {
    ...
    while (retryCount <= retryTimes) {
      ...
      try {
        ...
        if (resp.getStatus() ==200 || resp.getStatus()==500) {
          retryTimes = 0;
    ...
    throw new Exception("Failed in Connection Retry.");
  }
...
//    }
  ...
  private void handleErrorResponse(Response resp) throws Exception {

    if (!resp.isOK()) {
      ErrorMessage error = new ErrorMessage();
      try {
//        error = JAXBUtils.unmarshal(resp, ErrorMessage.class);
          //下面的部分可以封装为一个方法，由方法返回给error
        String resp_body = new String(resp.getBody(),"UTF-8");
        Map<String,Object>rets = JSONUtils.parseJSON2Map(resp_body);
        error.setCode(resp.getStatus());
        error.setSuccess((Boolean) rets.get("success"));
        error.setMsg((String) rets.get("msg"));
      } catch (Exception e) {
        //
      }
...
//        }
        ...
        if (error != null) {
          e = new Exception(error.getMsg(), new RestException(error));
        } else {
          e = new Exception(String.valueOf(resp.getStatus()));
        }
      }
      throw e;
    }
  ...
  private Request buildRequest(String resource, String method, Map<String, String> params,
    ...
    if (endpoint == null) {
      throw new Exception("Odps endpoint required.");
    }
